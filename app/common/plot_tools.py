...
def plot_heat_consumption():
    ...
    for loc in locations:
        ...
        data = running_consumption(data, time_series)
        bar_chart.add(loc, data)
    bar_chart.x_labels = [d.strftime("%Y-%m-%d") for d in time_series[1:]]
    return bar_chart.render(is_unicode=True)


def plot_water_consumption():
    topla_hladna = get_water_consumption("topla voda"), get_water_consumption("hladna voda")
    topla_hladna_str = ["topla voda", "hladna voda"]
    bar_chart = pygal.Line(width=600, height=300, explicit_size=True, style=CleanStyle)
    bar_chart.title = "Poraba vode [liter/dan]"
    for x, tp in enumerate(topla_hladna):
        data_in = get_water_consumption(topla_hladna_str[x])
        data = []
        time_series = []
        for d in data_in:
            data.append(d[0]*1000)
            time_series.append(d[1])
        bar_chart.add(topla_hladna_str[x], running_consumption(data, time_series))
    bar_chart.x_labels = [d.strftime("%Y-%m-%d %H:%M") for d in time_series[1:]]
    return bar_chart.render(is_unicode=True)
